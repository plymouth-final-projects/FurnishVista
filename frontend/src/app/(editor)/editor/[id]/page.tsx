...
import { OnboardingOverlay } from '@/components/shared/OnboardingTooltip';
...
export default function EditorPage() {
  ...
  const { designs, loadMockDesigns, updateDesign, addDesign } = useDesignStore();
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (designId === 'new' || !designs.find((d) => d.id === designId)) {
        const newDesign = {
          id: `design-${Date.now()}`,
          name: designName,
          room,
        ...
        window.history.replaceState(null, '', `/editor/${newDesign.id}`);
      } else {
        updateDesign(designId, { name: designName, room, furniture });
      }
  ...
  }, [designId, designs, designName, room, furniture, addDesign, updateDesign, setDesignId, setDirty]);
  ...
  const shortcuts = useMemo(
    () => [
      ...
      { key: 's', ctrl: true, handler: handleSave },
      {
        ...
        handler: () => {
          if (selectedItemId) {
            const itemData = mockFurniture.find(
